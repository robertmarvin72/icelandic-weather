...
Independently verified every "known live finding" in §1 against the live tree rather than trusting the prompt's narrative, then traced the full stale-classification → analytics/UI data flow named in §4 end to end:

- `vercel.json` (current cron entries)
- `api/cron/refresh-aurora.js`, `api/_lib/aurora/cache.js`
- `api/_lib/auroraDecision/constants.js`, `freshness.js`, `orchestrate.js`
- `src/lib/auroraDecisionClassify.js`
- `src/components/NorthernLightsCard.jsx` (full stale-event effect and stale-notice render path)
- `device_list_dir` on `api/_lib/auroraDecision`, `api/cron`, `api/_lib/aurora` to confirm every test file the prompt references actually exists.

## §1's "known live findings" — confirmed accurate, with one correction

- `vercel.json`'s single Aurora cron entry (`0 12,20 * * *`), `AURORA_FRESH_MAX_AGE_MINUTES=360`, `AURORA_STALE_MAX_AGE_MINUTES=1440`, `classifyAuroraCache`'s inclusive `source_fetched_at`-based boundaries, and the success/failure persistence split in `refresh-aurora.js`/`cache.js` all check out exactly as stated.
- All referenced test files exist (`freshness.test.js`, `refresh-aurora.test.js`, `cache.test.js`, `orchestrate.test.js`, etc.) — no phantom file references.
- **The one inaccurate claim**: "the visible stale notice is driven by that same canonical classification value [`classification.freshness === 'stale'`]." In `NorthernLightsCard.jsx`, the notice is actually gated by `isStale && staleAgo` (`AuroraResult`, lines ~315–316, consumed by `StaleParialNotices` at line 295/303), where `staleAgo = formatAgo(body.auroraCache?.sourceFetchedAt, t, nowMs)` — and `formatAgo` returns `null` (not just falls back) when the client-computed age is negative or non-finite. The analytics trigger (line 170, `if (classification.freshness === "stale")`) has no equivalent second gate. This distinction turns out to matter — see the required finding below, which is a direct, reachable consequence of it.

## Required finding: the stale event fires for non-result outcomes, not just for a usable stale result

`classification.freshness` is computed from `body.auroraCache?.state` for **every** primary outcome, not just `"success"`/`"partial"` (`auroraDecisionClassify.js` lines 57–67: the `freshness` value is attached identically whether `body.status` is `"success"`, `"partial"`, or `"unavailable"`). `orchestrate.js` confirms this is reachable: when the aurora cache itself is merely stale (not `"unavailable"`), execution proceeds past the cache-availability check and can still return an `"unavailable"` domain status for unrelated reasons — `night_not_found`, `invalid_darkness_window`, `no_locations_scored` — each of which still carries the real (stale) `auroraCache` in the response body. `classifyAuroraOutcome` then maps that to `primary: "domain_unavailable"` (or, for the darkness-window case, `"no_darkness"`) with `freshness: "stale"` intact.

In `NorthernLightsCard.jsx`, the render branches for `no_darkness` (line 257), `domain_unavailable` (line 264), `contract_defect` (line 248), and `transport_error` (line 239) are separate JSX blocks that never render `AuroraResult`/`StaleParialNotices` — `AuroraResult` is only reached when `isResultState` (`primary === "success" || primary === "partial"`, line 211/273) is true. But the analytics effect's stale branch (line 170) checks only `classification.freshness === "stale"`, with no primary/`isResultState` condition at all.

**Concretely: a stale-but-present aurora cache combined with `night_not_found` (a realistic, independent condition — the requested evening simply isn't in the cached snapshot) fires `northern_lights_stale_viewed` while the user is shown the plain "unavailable, retry" card with zero mention of staleness.** This is exactly the failure mode §4 names and forbids ("Do not create a state where the event fires without the notice"), it is reachable today (before this ticket), and the ticket's own extended overnight stale window (§2, the documented 20:00→08:00 gap) makes the stale-cache half of this combination more likely to occur, not less.

Required additions to the prompt:

1. The preflight audit (§1) must record this exact reachability chain (stale cache + `night_not_found`/`invalid_darkness_window`/`no_locations_scored` → `domain_unavailable`/`no_darkness` with `freshness: "stale"`) as a confirmed, not hypothetical, pre-existing gap.
2. §4's required fix: gate the `northern_lights_stale_viewed` trigger on the same `isResultState`-equivalent condition (`primary === "success" || primary === "partial"`) that already gates the visible notice, so the two can never disagree on whether a "usable resolved result" exists. This is a local, one-line change to the existing effect condition — well within §4's explicit "if a code change is genuinely required to align event firing with rendered exposure, keep it local" allowance, and does not touch request identity, cache semantics, result classification, UI copy, or analytics architecture.
3. Add to required test #15 (or as a new #16): a case with a stale (not unavailable) aurora cache combined with each of `night_not_found`/`invalid_darkness_window`-driven unavailability must render the existing non-stale unavailable/no-darkness UI **and must not emit `northern_lights_stale_viewed`**.

## Secondary, non-blocking note

Independently of the above, `formatAgo`'s `ms < 0` → `null` path means that even *within* a genuine `success`/`partial` result, a large enough client/server clock skew could make `staleAgo` fall back to `null` while `classification.freshness` is still `"stale"` — i.e. the same event-without-notice shape, but caused by clock drift rather than the primary-outcome gap above. If the required fix in point 2 is scoped only to the primary/`isResultState` condition (as intended), this narrower path remains theoretically open. Given `formatAgo` already treats a negative computed age as un-renderable rather than clamping it, I'd suggest the audit note whether this is realistically reachable under the project's actual clock source (server `now()` vs. client `Date.now()`) and either document it as accepted residual risk or fold the same `staleAgo`-truthy check into the event gate while it's already being touched. Not blocking — the required finding above is the one that must be fixed before implementation.

## Everything else in the prompt

The cron-schedule change, the 8-hour fresh threshold (with its correctly-inclusive boundary and explicit sub-minute-crossing test requirement #6), the hard-cutoff preservation, the successful/failed refresh invariants, and the hard scope boundaries are all precisely specified and match the live code and the Ticket 1/3 contracts they build on. No other gap found.

---

## Round 2 — Ripley revision

Jonesy's required finding is accepted. The following clarification amends Round 1 and is authoritative wherever the earlier stale UI/analytics wording could be read ambiguously. All other Round 1 scope, schedule, freshness, cache, tests, validation, acceptance criteria, and STOP conditions remain unchanged.
...
Re-checked Round 2 against the live code rather than accepting the revision text at face value:

- The proposed fix (`if (isResultOutcome && classification.freshness === "stale") { trackEvent(...) }`) reuses a variable that genuinely already exists at the exact call site: `NorthernLightsCard.jsx` line 153 already computes `const isResultOutcome = classification.primary === "success" || classification.primary === "partial";` inside the same effect, currently used only for the `resultState` field on `northern_lights_card_viewed`. Round 2 correctly identifies and reuses this rather than inventing a second classifier — exactly what I asked for.
- The reachability chain in "Confirmed pre-existing non-result stale-event mismatch" (steps 1–5) matches what I independently traced from `freshness.js` → `orchestrate.js` → `auroraDecisionClassify.js` → `NorthernLightsCard.jsx` in Round 1 — no embellishment, no new claims beyond what's verifiable.
- Required test case 5 ("`stale cache + no_locations_scored`—including the established domain-unavailable and unambiguous no-darkness classification shapes where applicable") correctly reflects `auroraDecisionClassify.js`'s actual branching: `no_locations_scored` maps to `no_darkness` only when every excluded location's status is `not_viewable_tonight`, and to `domain_unavailable` otherwise. Getting this nuance right in the test requirement (rather than treating `no_locations_scored` as a single fixed outcome) matters for real coverage.
- The clock-skew decision is correctly scoped: documented as accepted residual risk with its precise reachability condition, no production change authorized, and a STOP condition added if the required fix turns out to depend on it. This matches what I actually asked for — the secondary note was explicitly non-blocking in Round 1, and Round 2 treats it that way rather than either ignoring it or overreaching into a broader clock refactor.

## Outcome

Round 2 resolves the Round 1 required finding precisely, at the correct location, with the correct scope, and without touching anything outside what the finding required. No new gaps introduced. Approved to proceed to implementation.
