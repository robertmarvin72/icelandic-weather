...
Strong prompt overall — the row.code/summaryCode separation is exactly right and I independently confirmed why it matters (see below), the family/tie-break/override design is sound, and the preflight audit list is well-targeted. One required addition grounded in the live code, plus two lower-severity notes.

### Confirmed: `row.code` genuinely affects scoring — the core invariant is real, not just cautious boilerplate

Read `src/lib/scoring.js` directly: `scoreSiteDay` does `const _weatherCode = weatherCode ?? code ?? null;` then feeds it into `getPleasantnessModifier(...)`, which is added into `pointsRaw` — the actual score. So the prompt's insistence on keeping `row.code` untouched and never letting `summaryCode` reach `scoreSiteDay` is protecting a real, active dependency, not a hypothetical one. Good call requiring an explicit scoring-invariance regression test (§6.11).

Also confirmed the hourly endpoint already carries everything needed (`hourly.weathercode`, `hourly.precipitation`, `hourly.time` are all requested in both `api/forecast.js` and `forecastCache.js`'s Open-Meteo params, with a fixed `timezone: "Atlantic/Reykjavik"` — so the timestamps really are timezone-local, not UTC, as §3 assumes) and that the `hourly.weather_code` defensive-alias precedent (§3, "Input handling") is real: `HourlyForecastModal.jsx`'s `buildHourlyRows` already does `hourly?.weathercode?.[i] ?? hourly?.weather_code?.[i] ?? null` (and the same dual-naming pattern for wind fields). Nothing invented here.

### Required: name the two exact places today's "silently becomes clear sky" bug lives

§4 says unknown/missing codes "must never silently display as clear sky or overcast due to numeric coercion/fallback" — this is correct, but I traced the actual current violations and the prompt doesn't point CC at them, which risks the new shared mapping being safe in isolation while the existing unsafe call sites keep coercing before ever reaching it:

1. `src/components/ForecastTable.jsx` lines 277 and 402 both do `const code = Number(r.code ?? 0);` — a missing/null `row.code` is silently coerced to `0`, which `weatherMap[0]` renders as **clear sky**. This is the literal anti-pattern §4 warns about, live today, and it happens *before* any lookup — a fallback table with a safe "unknown" entry never even gets a chance to apply unless this coercion itself is removed.
2. `src/utils/WeatherIconMapping.ts`'s `mapWeatherCodeToIconId` falls through to `return "cloudy"` (line 73) for any code it doesn't explicitly recognize — and `"cloudy"` is the exact same icon ID it returns for code `3` (overcast) explicitly. So an unmapped/unknown code today silently renders with the **same icon as overcast**, which is the other half of §4's warning.

Useful precedent already in the repo: `HourlyForecastModal.jsx`'s `getWeatherInfo` already does this correctly — `if (code == null) return { icon: "—", label: "Unknown weather" }`, and an unrecognized-but-present code falls back to a labeled `"Weather {code}"` rather than silently becoming a real condition. The new shared mapping should generalize *this* existing pattern, not the daily table's. Please add a sentence naming these two call sites (or the equivalent instruction to audit-and-fix every existing `?? 0` / silent-default call site feeding weather-code display, not only build a new table that assumes it will always be called safely).

### Note (non-blocking): "preserving meaningful intensity" needs one more sentence

§3's representative-code rule says to select a deterministic representative code "preserving meaningful intensity when supported by the observations," and separately defines a position-based tie-break (center-of-window, never array order). It's not stated how these two interact when a winning family contains genuinely different-severity codes at different hours (e.g., light rain at 08:00, heavy rain at 14:00) — does intensity ever override the position-based pick, or is position always primary and "intensity" only means "pick the specific code that occurred nearest the window center, whatever its severity"? Either answer is defensible; recommend one clarifying sentence plus (optionally) a test case with mixed severities in one family, since §6 doesn't currently have one.

### Note (non-blocking): precipitation-duration text and `summaryCode` consistency

`ForecastTable.jsx` currently gates its precipitation-duration text (`getPrecipitationLabel`) on locally-duplicated `SNOW_CODES`/`RAIN_CODES` arrays checked against raw `row.code` — separate from the `precipType` field `forecastNormalize.js` already computes per row from the same raw code. §4's last bullet ("remove duplicated local WMO code lists where the shared source supersedes them") already licenses cleaning this up; worth explicitly confirming in implementation whether that sub-text should key off the new `summaryCode`'s resolved family so the headline and its own precipitation caption can't disagree with each other — a narrower echo of the exact bug this ticket fixes between daily and hourly.

Please address the required item, then resubmit for Round 2. The two notes are judgment calls for Ripley/CC, not blockers.
...
All three Round 1 items are resolved precisely and correctly, with no scope creep — Round 1's hard invariants (§2, §10) are untouched.

1. **Unsafe fallbacks** — both named call sites get concrete, correct instructions (`r.summaryCode ?? r.code ?? null` null-preserving semantics; the `WeatherIconMapping.ts` "cloudy"-for-unknown ambiguity explicitly called out and forbidden), `HourlyForecastModal.jsx#getWeatherInfo` is correctly identified as the pattern to generalize, and — importantly — the required tests must "exercise the component call sites as well as the pure resolver," which is exactly what closes the gap I raised (a safe helper that an unsafe caller bypasses). The "search for equivalent instances but don't broaden scope" instruction is a good, disciplined touch.

   One additional data point worth having on record, not a new blocker: I checked `WeatherIcon.tsx` itself and found a *third* layer of the same pattern — `ICONS[iconId] ?? IconCloudy`, so even a synthetic "unknown" `iconId` string would silently render the cloudy icon unless something else intervenes. The existing `WeatherIconId` union also has no neutral/unknown member, and Round 1's §4 forbids new icon artwork. Round 2's "use a small accessible neutral fallback at the component boundary" instruction — i.e., don't route the unknown case through `<WeatherIcon>` at all — already sidesteps this correctly by construction, even without naming this exact line, so no further prompt change is needed. Flagging it so CC's own audit isn't the first time this is noticed.

2. **Representative intensity** — the precedence is now fully specified (intensity first, position only tie-breaks equal-intensity observations, never invent an unobserved code) and §4's ordering-safety concern I hadn't even explicitly raised — that a high-intensity representative can't retroactively let a non-dominant family bypass the override threshold — is explicitly foreclosed in point 4. The added mixed-intensity test matches what I asked for.

3. **Precipitation caption consistency** — now correctly keyed off the shared resolver's family for the *displayed* code, explicitly handles both directions (precip-summary-over-dry-raw-code and dry-summary-over-precip-raw-code), and protects against mislabeling sleet/freezing precip just to reuse the caption helper. Bidirectional test coverage was added.
