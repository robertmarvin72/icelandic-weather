...
Reviewed against the live working tree, not from memory: `docs/ai/tasks/ticket-392/result-review.md` (the owner-approved six-ID disclosure exception this prompt must preserve unexpanded); `src/components/NorthernLightsCard.jsx` and its test; `src/components/NorthernLightsMap.jsx` and its test; `src/MapView.jsx`; `src/lib/auroraDecisionClassify.js`; `api/_lib/auroraDecision/rankDecision.js` and `orchestrate.js`; `src/config/auroraCandidates.js`; `src/i18n/translations.northernLights.js`.

### Verified as sound (no changes requested)

- **The "known audit finding" in §1 is exactly right, confirmed against source, not asserted.** `NorthernLightsMap.jsx` passes bare `{id, name, lat, lon}` (no band) into the generic `MapView`, which independently `fetchForecastAndScore`s each site via `getForecast`/`scoreSiteDay` and buckets into `Good`/`Fair`/`Rough` off a 0–70 weekly campsite score (`MapView.jsx` L16–44) — a completely different scale from Aurora's canonical `excellent/good/fair/poor/very-poor` band. This is a real, verified second-fetch/second-scoring bug, not a hypothetical one, and it directly justifies §6's ban on re-fetching/re-scoring for the Aurora map path.
- **The "fixed six-place checklist" and "awkward subtitle" complaints are real, not just the issue's framing.** `AuroraResult` in `NorthernLightsCard.jsx` today maps over all of `rankedLocations` unconditionally (no band filter) whenever `detailsExpanded`, and `nlCardSubtitle`/`nlAlternativesHeading` read exactly as described ("A quick, secondary check — not the main recommendation above." / "Other locations checked tonight") — confirmed directly in `translations.northernLights.js`.
- **§7's analytics-truthfulness ask is a real, verified gap, not precautionary.** `northern_lights_ranking_viewed`/`northern_lights_map_viewed` currently fire off `isPro && detailsExpanded && requestKey` alone (`NorthernLightsCard.jsx` L108–122) — with no check that a ranking or map is actually rendered. Under today's code this already slightly overfires (map is always rendered when expanded); under the new state model it would fire even when the list/map is correctly hidden (all-poor, one qualifying entry, same-band-only). §7's requirement is squarely necessary, not speculative.
- **The `best`-non-null invariant §3/§4B silently depend on is genuinely guaranteed, not assumed.** Traced the full path: `rankDecision.js`'s `buildRankedDecision` returns `best: null` only when zero locations were successfully scored; `orchestrate.js` L99–109 explicitly routes that exact case to `status: "unavailable", reason: "no_locations_scored"` *before* the `status: excluded.length > 0 ? "partial" : "success"` branch is ever reached (L124–125) — so `body.status` can only be `"success"`/`"partial"` when `best` is non-null. `classifyAuroraOutcome` passes `body.status` straight through. This means the new "all-poor" state (§4B) can never actually encounter a null `bestAvailable` — the prompt's design is safe on a point it never has to spell out, because the underlying contract already enforces it.
- **§2's "keep Ticket 392's six-ID exception exactly bounded" is consistent with what was actually approved**, not a mischaracterization: ticket-392's `result-review.md` records Róbert's and Ripley's explicit sign-off on exactly the six fixed OSM IDs reaching Free's bundle/request while names/coordinates/scores/reasons/ranking/map stay gated — this prompt changes none of that boundary, and §2's "do not expand it or expose Pro names, coordinates, scores, reasons, ranking, or map data to Free" matches the approved exception's own conditions word-for-word.
- **The six-candidate roster (`auroraCandidates.js`) is confirmed to be exactly six**, so "cap at six" in §3/§9 is the actual ceiling of the fixed roster, not an arbitrary number that could truncate real qualifying results.
- **§6's MapView adapter ask is appropriately hard-STOPped, not underspecified.** `MapView.jsx` has no existing externally-supplied-score/mode prop — adding one is genuine, non-trivial design work on a shared component (also used by the main campsite map and `RouteCompareTable`). The prompt doesn't prescribe the adapter shape (correctly, since that's an implementation decision within a presentation-only boundary) but backs it with an explicit, specific STOP condition ("broad generic-map regressions, duplicated Leaflet architecture, a new dependency, or any re-scoring") — the same audit-first/STOP-before-guessing discipline this workflow has required on every prior ticket.
- **Free's current all-poor exposure risk is real and correctly targeted.** `nlFreeHint`/`nlUpgradeCta` ("Conditions may be worth checking somewhere in Iceland tonight." / "See where and why (Pro)") are shown unconditionally to Free today regardless of band — exactly the misleading-when-poor copy §4B requires be replaced with honest, non-teasing guidance.

### Findings

None. This is a well-audited prompt: every factual claim it makes about current behavior checks out against the live source, the one genuinely hard design problem (the MapView adapter) is bounded by an explicit STOP rather than hand-waved, and the ticket correctly treats ticket-392's owner-approved disclosure boundary as closed rather than reopening it.

### Verdict: APPROVED

No revision needed. Ripley: please create `approved-prompt-v1.md` and set `CURRENT.md → READY_FOR_CC` per `docs/ai/README.md`.
