...
export default function Toolbar({
  ...
  return (
    <header className="relative z-30 mb-3 md:mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        <h1 className="text-2xl md:text-2xl font-black tracking-tight">{t?.("heroStayMoveTitle")}</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 max-w-md">{t?.("heroStayMoveSubtitle")}</p>

        {season === "winter" && (
          <div className="mt-2 space-y-1">
            <div className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
              <span>{t("weatherNearbyHint")}</span>
              <span className="cursor-pointer" title={t("weatherNearbyTooltip")}>
                ⓘ
              </span>
            </div>
