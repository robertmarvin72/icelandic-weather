...
export default function NorthernLightsCard({ t, lang, entitlements, onUpgrade, theme, fetchImpl, now }) {
  ...
  }, [classification, requestKey, isPro]);

  const rankingViewedRef = useRef(null);
  useEffect(() => {
    if (!isPro || !detailsExpanded || !requestKey) return;
  ...
  }, [isPro, detailsExpanded, requestKey]);
  ...
  const rankedLocations = useMemo(() => {
    if (!classification?.body) return [];
    const best = classification.body.best;
    const alternatives = Array.isArray(classification.body.alternatives) ? classification.body.alternatives : [];
...
function AuroraResult({ t, lang, theme, classification, isPro, detailsExpanded, onToggleDetails, onUpgrade, rankedLocations, nowMs }) {
  const body = classification.body;
  const best = body.best;
  const bandKey = BAND_KEYS[best?.band] || BAND_KEYS.fair;
        ...
        <div className="mt-2">
          ...
          {detailsExpanded && (
            <div id="nl-details-panel" className="mt-3 space-y-3">
              ...
              <div>
                <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t("nlAlternativesHeading")}</h3>
                <ol className="mt-1 space-y-1 text-xs text-slate-700 dark:text-slate-300" aria-label={t("nlAlternativesHeading")}>
                  {rankedLocations.map((loc, idx) => (
                    <li key={loc.locationId} className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 px-2 py-1 dark:border-slate-700">
                      ...
                      <span className="text-slate-500 dark:text-slate-400">{t(BAND_KEYS[loc.band] || BAND_KEYS.fair)}</span>
                    </li>
                  ))}
                </ol>
              ...
              <NorthernLightsMap
                locations={rankedLocations.map((l) => ({ id: l.locationId, name: l.name, lat: l.lat, lon: l.lon }))}
